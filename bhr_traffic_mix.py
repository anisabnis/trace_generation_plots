...
def plot_orig(arg1 , arg2, label, marker, every):
    if label == "orig":
        if arg2 == "rhr":
            ...
            f = open(d + "/" + arg1 + "/" + "calculus_byte_footprint_desc_" +str("mix") + ".txt", "r")
    else:
        if arg2 == "rhr":
            f = open(d + "/" + arg1 + "/generated/" + "footprint_desc_" +str("mix") + ".txt", "r")        
        else:
            f = open(d + "/" + arg1 + "/generated/" + "byte_footprint_desc_" +str("mix") + ".txt", "r")        
    ...
    plot_dict(prs, one_hits_pr, arg2+"-"+label, marker, every)
...
i = 0
plot_orig("eu", "rhr", "gen", markers[i], 10000)
i+= 1
plot_orig("eu", "rhr", "orig", markers[i], 10000)
i+= 1
plot_orig("eu", "bhr", "gen", markers[i], 10000)
i+= 1
plot_orig("eu", "bhr", "orig", markers[i], 10000)
i+= 1



plt.ylabel("Hitrate (hr)")
plt.xlabel("Cache size (TB)")
plt.grid()
plt.legend()
plt.gcf().subplots_adjust(left=0.15, bottom=0.15)
plt.ylim([0.,1])
plt.savefig("EU_mix.pdf")
