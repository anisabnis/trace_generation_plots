...
def plot_orig(arg1, arg2,label, marker, every):
    if arg1 == "":
        if label == "original":
            f = open(d + "/tc/" + str(arg1) + "footprint_desc_" +str(arg2) + ".txt", "r")
        elif label == "calculus":
            f = open(d + "/tc/calculus/" + str(arg1) + "footprint_desc_" +str(arg2) + ".txt", "r")
    else:
        f = open(d + "/tc/naive_merge/" + str(arg1) + "footprint_desc_" +str(arg2) + ".txt", "r")        
...
i = 0
plot_orig("", "01", "calculus", markers[i], 800)
i+= 1
plot_orig("", "01", "original", markers[i], 600)
i+= 1
plot_orig("o_", "01", "merge-req", markers[i], 400)
i += 1
plot_orig("b_", "01", "merge-bytes", markers[i], 700)
i += 1
plot_orig("u_", "01", "merge-uniq-bytes", markers[i], 500)
